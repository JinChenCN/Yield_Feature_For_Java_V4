...
public final class DumpVisitor implements VoidVisitor<Object> {
	...
	List<Parameter> expectParameters = null;
	List<Expression> actualParameters = null;

	private String replaceParams(List<Expression> actualParameters, List<Parameter> expectParameters, String s) {
		if(actualParameters == null)
		{
			throw new A2SemanticsException("Parameter of " + newMethodName + " cannot be null.");
		}
		String newString = s;
		...
		{
			String pBeReplaced = actualParameters.get(i).toString();
			String pExpected = expectParameters.get(i).getId().toString();
			if (list.contains(pBeReplaced)){
				Collections.replaceAll(list, pBeReplaced, pExpected);
    ...
    public void visit(YieldStmt n, Object arg) {
		...
		HashMap<String, BlockStmt> yieldMap = YieldStore.getBlocks(currentMethod);
    	if(yieldMap != null)
    	{
            BlockStmt yieldBlock = yieldMap.get(methodName);
            if(yieldBlock != null)
            {
            	expectParameters = YieldStore.getParams(currentMethod);
            	if (expectParameters != null && expectParameters.size() > 0)
            	{
            		actualParameters = YieldStore.getYieldParams(newMethodName);            		
            	}
                isYieldBlock = true;
                yieldBlock.accept(this, arg);
                isYieldBlock = false;
                expectParameters = null;
    		...
    		printer.print("//yieldMap is null!");
    	}
    }
    ...
    public void visit(MethodDeclaration n, Object arg) {
		...
		printMethod(n, arg);
    	HashMap<String, BlockStmt> yieldMap = YieldStore.getBlocks(originalName);
    	if( yieldMap != null)
    	{
    		for (int i = 1; i < yieldMap.size(); i++)
    		{   			
    			n.setName(originalName+"Yield"+i);
    			printMethod(n, arg);
    		}
			currentMethod = "";
    	} 
    ...
    private void printMethod(MethodDeclaration n, Object arg) {
    	if (n.getJavaDoc() != null) {
            n.getJavaDoc().accept(this, arg);
        }
        printMemberAnnotations(n.getAnnotations(), arg);
