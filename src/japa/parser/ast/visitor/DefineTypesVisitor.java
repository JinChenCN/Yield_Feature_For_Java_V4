		...
		if (currentScope instanceof ClassSymbol) {
			...
			if (n.getImplements() != null) {
				...
				for (ClassOrInterfaceType c : n.getImplements()) {
					Symbol interfaceSym = currentScope.resolve(c.getName());
					if (interfaceSym instanceof InterfaceSymbol) {
						InterfaceSymbol resolvedInterfaceSym = (InterfaceSymbol) interfaceSym;
						implementsList.add(resolvedInterfaceSym);
					} else {
						throw new A2SemanticsException(c.getName() + " in class "
			...
			if (n.getExtends() != null) {
				for (ClassOrInterfaceType c : n.getExtends()) {				
					Symbol classSym = currentScope.resolve(c.getName());
					if (classSym instanceof ClassSymbol) {
						ClassSymbol resolvedClass = (ClassSymbol) classSym;						
			...
			if (n.getExtends() != null) {
				for (ClassOrInterfaceType c : n.getExtends()) {
					Symbol interfaceSym = currentScope.resolve(c.getName());
					if (interfaceSym instanceof InterfaceSymbol) {
						InterfaceSymbol resolvedInterface = (InterfaceSymbol) interfaceSym;
						interfaceSymbol.setSuperinterface(resolvedInterface);
					} else {
						throw new A2SemanticsException(c.getName() + " in interface "
								+ n.getName() + " on line " + n.getBeginLine()
								+ " is not a valid interface.");
					}
				}
